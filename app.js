...
const WARD_CONFIG = {
  ...
  '渋谷区':   { center: [35.6614, 139.7036], zoom: 13 },
  '板橋区':   { center: [35.7530, 139.7090], zoom: 13 },
  '府中市':   { center: [35.6694, 139.4773], zoom: 13 },
  '墨田区':   { center: [35.7100, 139.8010], zoom: 13 }
};
...
const TOWN_COORDS = {
  ...
  // 渋谷区
  '恵比寿':     [35.6467, 139.7098],
  '広尾':       [35.6492, 139.7190],
  '代官山':     [35.6489, 139.7033],
  '渋谷':       [35.6580, 139.7016],
  '松濤':       [35.6620, 139.6960],
  '神宮前':     [35.6694, 139.7075],
  '千駄ヶ谷':   [35.6720, 139.7130],
  '富ヶ谷':     [35.6650, 139.6890],
  '初台':       [35.6840, 139.6940],
  '本町':       [35.6790, 139.6870],
  '笹塚':       [35.6750, 139.6760],
  '幡ヶ谷':     [35.6720, 139.6800],
  '代々木':     [35.6830, 139.7000],
  // 板橋区
  '板橋':       [35.7490, 139.7100],
  '大山':       [35.7550, 139.7050],
  '常盤台':     [35.7620, 139.6940],
  '成増':       [35.7760, 139.6660],
  '高島平':     [35.7750, 139.6980],
  '志村':       [35.7700, 139.7080],
  '前野町':     [35.7640, 139.7150],
  '東武練馬':   [35.7690, 139.6850],
  '上板橋':     [35.7620, 139.7050],
  '小茂根':     [35.7580, 139.6980],
  '加賀':       [35.7520, 139.7160],
  // 府中市
  '府中':       [35.6694, 139.4773],
  '宮西町':     [35.6720, 139.4830],
  '是政':       [35.6550, 139.4720],
  '白糸台':     [35.6580, 139.4850],
  '住吉町':     [35.6750, 139.4900],
  '武蔵台':     [35.6830, 139.4820],
  '東府中':     [35.6640, 139.4960],
  '多磨町':     [35.6860, 139.5050],
  '府中新町':   [35.6710, 139.4700],
  '分梅町':     [35.6660, 139.4800],
  '若松町':     [35.6780, 139.4770],
  '小柳町':     [35.6700, 139.4850],
  // 墨田区
  '錦糸町':     [35.6960, 139.8130],
  '両国':       [35.6960, 139.7930],
  '押上・スカイツリー': [35.7100, 139.8120],
  '向島':       [35.7150, 139.8090],
  '東向島':   [35.7200, 139.8060],
  '墨田':       [35.7180, 139.8180],
  '菊川・緑':     [35.6940, 139.8000],
  '吾妻橋・横川': [35.7060, 139.8050],
  '京島・文花':   [35.7070, 139.8200],
  '八広・立花':   [35.7200, 139.8250]
};
...
function selectWard(ward) {
  ...
  document.getElementById('search-input').placeholder =
    ward === '江東区' ? '町名を検索（例：豊洲、清澄白河...）' :
    ward === '渋谷区' ? '町名を検索（例：恵比寿、代官山、原宿...）' :
    ward === '板橋区' ? '町名を検索（例：大山、常盤台、高島平...）' :
    ward === '府中市' ? '町名を検索（例：府中、是政、白糸台...）' :
    ward === '墨田区' ? '町名を検索（例：錦糸町、押上、両国...）' :
    '町名を検索（例：三軒茶屋、下北沢...）';
}
